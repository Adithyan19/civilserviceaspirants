...
const AdminPanel: React.FC<AdminPanelProps> = ({ onLogout }) => {
  ...
  const [loadingPendingArticles, setLoadingPendingArticles] = React.useState(false);
  const [processingArticleId, setProcessingArticleId] = React.useState<string | null>(null);
  ...
  const fetchPendingArticles = async () => {
    ...
    try {
      const res = await api.get<PendingArticle[]>("/api/admin/articles/pending", {
        headers: { Authorization: `Bearer ${token}` },
      });
      setPendingArticles(res.data || []);
    } catch {
      showError("Failed to fetch pending articles.");
    } finally {
      setLoadingPendingArticles(false);
    }
  };

  const handleArticleAction = async (id: string, action: "approved" | "rejected") => {
    ...
    try {
      ...
      showSuccess(`Article ${action} successfully!`);
      setPendingArticles((prev) => prev.filter((a) => a.id !== id));
    } catch {
      showError(`Failed to ${action === "approved" ? "approve" : "reject"} article.`);
    } finally {
  ...
  const renderArticleApproval = () => (
    <div className="space-y-6">
      <h3 className="text-xl font-semibold text-gray-800 mb-4">
        Article Approval
      </h3>
      {loadingPendingArticles ? (
        <p className="text-gray-500 text-center py-8">Loading pending articles...</p>
      ) : pendingArticles.length === 0 ? (
        <div className="text-center py-12 bg-white rounded-lg border border-gray-200">
          <CheckCircle className="w-12 h-12 text-green-400 mx-auto mb-4" />
          <p className="text-gray-500 text-lg">No pending articles to review</p>
          ...
          {pendingArticles.map((article) => (
            ...
            >
              <div className="flex items-start justify-between mb-3">
                ...
                <span className="px-2.5 py-1 bg-yellow-100 text-yellow-700 text-xs font-medium rounded-full flex-shrink-0 ml-3">
                  Pending
                </span>
              </div>
              ...
              <div className="flex items-center space-x-3 pt-3 border-t border-gray-100">
                <button
                  ...
                  className="flex items-center space-x-2 px-4 py-2 bg-green-50 hover:bg-green-100 text-green-700 border border-green-200 rounded-md text-sm font-medium transition-colors disabled:opacity-50"
                >
                ...
                <button
                  ...
                  disabled={processingArticleId === article.id}
                  className="flex items-center space-x-2 px-4 py-2 bg-red-50 hover:bg-red-100 text-red-700 border border-red-200 rounded-md text-sm font-medium transition-colors disabled:opacity-50"
                >
